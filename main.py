...
pairs_state = {
    "btcidr": {
        "name": "BTC/IDR",
        "symbol": "btc",
        "allocation_ratio": 0.75,  # Alokasi 75%
        "is_running": True,
        "in_position": False,
        "buy_price": 0.0,
        "total_trades": 0,
    ...
    "usdtidr": {
        "name": "USDT/IDR",
        "symbol": "usdt",
        "allocation_ratio": 0.25,  # Alokasi 25%
        "is_running": True,
        "in_position": False,
        "buy_price": 0.0,
        "total_trades": 0,
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as resp:
            ...
            if res.get("success") == 1:
                ...
                grand_total = idr_cash + btc_val + usdt_val + other_coins_total_val
                
                if grand_total > 1000:
                    if global_state["peak_equity"] == 0.0 or grand_total > global_state["peak_equity"]:
                        global_state["peak_equity"] = grand_total
                    
                return True, idr_cash, btc_amt, usdt_amt, btc_val, usdt_val, other_balances_list, grand_total, "OK"
            else:
...
def check_initial_positions():
    success, _, btc_amt, usdt_amt, _, _, _, _, _ = fetch_realtime_account()
    if success:
        if btc_amt > 0.0001:
...
def get_home_text(is_final=False):
    success, idr_bal, btc_amt, usdt_amt, btc_val, usdt_val, other_list, total_equity, err = fetch_realtime_account()
    if not success:
    ...
    return (
        ...
        f" • 🪙 Aset BTC        : Rp {btc_val:,.2f} ({btc_amt:,.8f} BTC)\n"
        f" • 💵 Aset USDT       : Rp {usdt_val:,.2f} ({usdt_amt:,.2f} USDT)"
        f"{other_text}\n"
        f"{target_progress}\n"
        f"Jam : ⏱ {now_wib}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"1️⃣ BTC/IDR (75%) | {btc_p['price_trend']}\n"
        f"Status: {btc_status}\n"
        f"• Harga: Rp {btc_price:,.0f}\n"
        f"• Aset: Rp {btc_val:,.2f} ({btc_amt:,.8f} BTC)\n"
        f"• Grafik: {btc_chart}\n"
        f"• Posisi: {btc_pos}\n"
        ...
        f"```text\n{btc_logs}\n```\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"2️⃣ USDT/IDR (25%) | {usdt_p['price_trend']}\n"
    ...
    while True:
        try:
            ...
            if p_data["is_running"]:
                success, idr_cash, btc_amt, usdt_amt, _, _, _, current_equity, err = fetch_realtime_account()
                    ...
                    if current_equity < global_state["peak_equity"]:
                        ...
                        if drawdown >= MAX_DRAWDOWN_PCT:
                            ...
                            if p_data["in_position"]:
                                ...
                                min_check = 0.0001 if pair_key == "btcidr" else 0.00001
                ...
                if success and current_price > 0:
                    if not p_data["in_position"]:
                        available_idr = idr_cash - MIN_IDR_RESERVE
                        # Menghitung alokasi dana berdasarkan rasio pasangan (75% BTC / 25% USDT)
                        allowed_buy_idr = available_idr * p_data["allocation_ratio"]
                        
                        if allowed_buy_idr >= 10000:
                            add_log(pair_key, f"BUY dengan dana Rp {allowed_buy_idr:,.0f} (Alokasi {int(p_data['allocation_ratio']*100)}%)...")
                        ...
                        if price_change_pct >= 0.003 or price_change_pct <= -0.005:
                            success, _, current_btc, current_usdt, _, _, _, _, _ = fetch_realtime_account()
...
def handle_update(update):
    if "callback_query" in update:
        ...
        if data == "toggle_btcidr":
            ...
            success, idr, btc, usdt, btc_val, usdt_val, other_list, eq, _ = fetch_realtime_account()
            other_str = f"\n• Koin Lain: {', '.join(other_list)}" if other_list else ""
            status_text = (
                f"📊 *STATUS & SALDO AKUN LENGKAP*\n\n"
                f"• IDR Tunai: Rp {idr:,.2f}\n"
                f"• Saldo BTC: {btc:,.8f} BTC (Rp {btc_val:,.2f})\n"
            ...
            report_text = (
                ...
                f"**BTC/IDR (75%):** Trade: {b['total_trades']}x | Win: {b['winning_trades']} | Lose: {b['losing_trades']}\n"
                f"**USDT/IDR (25%):** Trade: {u['total_trades']}x | Win: {u['winning_trades']} | Lose: {u['losing_trades']}"
            )
