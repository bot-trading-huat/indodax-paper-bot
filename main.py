...
# Menyimpan data chat & message id untuk dikoreksi otomatis
active_dashboards = {}
...
def fetch_realtime_account():
    # 1. Ambil Ticker BTC & USDT Realtime (No Cache)
    btc_price = 0.0
    usdt_price = 0.0
    
    try:
        req = urllib.request.Request(f"https://indodax.com/api/ticker/btcidr?ts={int(time.time())}", headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=3) as resp:
            btc_price = float(json.loads(resp.read().decode("utf-8")).get("ticker", {}).get("last", 0))
    except: pass

    try:
        req = urllib.request.Request(f"https://indodax.com/api/ticker/usdtidr?ts={int(time.time())}", headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=3) as resp:
            usdt_price = float(json.loads(resp.read().decode("utf-8")).get("ticker", {}).get("last", 0))
    except: pass

    # 2. Ambil Saldo dari Private API
...
def build_dashboard():
    ...
    text += f"━━━━━━━━━━━━━━━━━━━\n"
    text += f"⏱ *Live Sync (Auto 5s):* `{now} WIB`"
    return text

# Loop background untuk auto-update pesan Telegram setiap 5 detik
        ...
        try:
            for chat_id, msg_id in list(active_dashboards.items()):
                telegram("editMessageText", {
                    ...
                    "parse_mode": "Markdown",
                    "reply_markup": {"inline_keyboard": [[{"text": "🔄 Refresh Manual", "callback_data": "btn_refresh"}]]}
                })
        except Exception:
            pass
        time.sleep(5)
...
def polling():
    ...
    # Jalankan Auto Update di background
    threading.Thread(target=auto_update_loop, daemon=True).start()
    print("Bot Indodax Real-Time Auto Refresh Aktif...")
